...
// Cross-language golden parity: for each committed golden/specs/*.json, prove over
// the shared golden/data.json and assert the response equals
...
public class GoldenTests
{
    ...
    [Fact]
    public void GoldenProofs_AreByteIdentical()
    {
        ...
        foreach (string specPath in Directory.GetFiles(Path.Combine(golden, "specs"), "*.json"))
        {
            ...
            using JsonDocument spec = JsonDocument.Parse(specText);
            ...
            string cmd = JsonSerializer.Serialize(new { cmd = "prove", spec = spec.RootElement, data = data.RootElement });
            string raw = prover.Command(cmd);
