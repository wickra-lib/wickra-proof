//! The wickra-proof C ABI — the hub every C-capable language links against.
//!
//! The surface is deliberately tiny and JSON-shaped, exactly like
//! `Prover::command_json`: create a stateless prover handle, drive it with
//! command JSONs (`prove`, `verify`, `canonicalize`, `version`) and read back
//! response JSONs, then free the handle. No proof type crosses the boundary by
//! value — the handle is opaque and the payloads are always UTF-8 JSON strings.
//!
...
//!
//! 1. Call [`wickra_proof_command`] with `out = NULL`, `cap = 0` to learn the
...
//! for unusable arguments ([`WICKRA_PROOF_ERR_NULL`], [`WICKRA_PROOF_ERR_UTF8`])
//! and caught panics ([`WICKRA_PROOF_ERR_PANIC`]); a non-negative return is
//! always the response length. Domain errors (a bad spec, an unknown command)
//! are *not* negative — they come back in-band as `{"ok":false,"error":...}`
//! JSON in the buffer.
...
pub const WICKRA_PROOF_ERR_NULL: i32 = -1;
/// `cmd_json` was not valid UTF-8.
pub const WICKRA_PROOF_ERR_UTF8: i32 = -2;
/// A panic was caught at the FFI boundary.
pub const WICKRA_PROOF_ERR_PANIC: i32 = -3;

/// An opaque handle to a prover instance. Created by [`wickra_proof_new`] and
/// destroyed by [`wickra_proof_free`]; never dereferenced by the caller.
pub struct WickraProof(Prover);
...
/// Construct a stateless prover handle.
///
/// Never fails; free the handle with [`wickra_proof_free`].
#[no_mangle]
pub extern "C" fn wickra_proof_new() -> *mut WickraProof {
    Box::into_raw(Box::new(WickraProof(Prover::new())))
}

/// Destroy a prover handle. Null is a no-op.
///
/// # Safety
/// `handle` must be null or a handle previously returned by
/// [`wickra_proof_new`] and not already freed.
#[no_mangle]
pub unsafe extern "C" fn wickra_proof_free(handle: *mut WickraProof) {
    if !handle.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_proof_command(
    handle: *mut WickraProof,
    cmd_json: *const c_char,
    out: *mut c_char,
    cap: usize,
) -> i32 {
    if handle.is_null() || cmd_json.is_null() {
        return WICKRA_PROOF_ERR_NULL;
    }
    let Some(cmd) = (unsafe { opt_str(cmd_json) }) else {
        return WICKRA_PROOF_ERR_UTF8;
    };
    let prover = unsafe { &mut (*handle).0 };
    let response = match catch_unwind(AssertUnwindSafe(|| prover.command_json(cmd))) {
        ...
        Err(_) => return WICKRA_PROOF_ERR_PANIC,
    };
...
#[no_mangle]
pub extern "C" fn wickra_proof_version() -> *const c_char {
...
mod tests {
    ...
    fn new_command_free_round_trip() {
        ...
        assert!(response.contains("\"engine_version\""));

        unsafe { wickra_proof_free(handle) };
    }
    ...
    fn bad_command_reports_error_in_band() {
        ...
        let len = unsafe { wickra_proof_command(handle, bad.as_ptr(), ptr::null_mut(), 0) };
    ...
    fn null_guards_on_command() {
        ...
        // Null handle.
        let code =
            unsafe { wickra_proof_command(ptr::null_mut(), cmd.as_ptr(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_PROOF_ERR_NULL);
        // Null command with a valid handle.
        let handle = wickra_proof_new();
        let code = unsafe { wickra_proof_command(handle, ptr::null(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_PROOF_ERR_NULL);
        unsafe { wickra_proof_free(handle) };
    }
    ...
    fn free_null_is_a_noop() {
        unsafe { wickra_proof_free(ptr::null_mut()) };
    }

    #[test]
    fn version_is_nul_terminated() {
        let p = wickra_proof_version();
