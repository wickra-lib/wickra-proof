 ...
 * {@code verify}, {@code canonicalize}, {@code version}) and read back the
...
public final class Prover implements AutoCloseable {
    ...
    public Prover() {
        try {
            ...
            if (created.address() == 0) {
                throw new IllegalStateException("wickra-proof: failed to create a prover");
            }
            this.handle = created;
    ...
    public String command(String cmdJson) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (len < 0) {
                throw new IllegalStateException("wickra-proof: command failed (code " + len + ")");
            }
